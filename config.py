...
PROCESSING_SETTINGS = {
    ...
    "PAGE_PROCESSING_TIMEOUT": 600,  # Увеличено до 10 минут
    
    # Таймаут для LLM запросов (в секундах)
    "LLM_REQUEST_TIMEOUT": 300,  # Увеличено до 5 минут
    
    # Максимальное количество попыток для LLM запроса
    "LLM_MAX_RETRIES": 5,  # Увеличено количество попыток
    
    # Интервал между попытками (в секундах)
    "LLM_RETRY_INTERVAL": 10,  # Увеличено время между попытками
    
    # Настройки памяти для обработки больших документов
    "MAX_MEMORY_USAGE_PERCENT": 85,  # Увеличено использование памяти
    
    # Размер буфера для обработки страниц (в байтах)
    "PAGE_BUFFER_SIZE": 20 * 1024 * 1024,  # Увеличено до 20 МБ
    
    # Максимальное количество страниц в памяти одновременно
    "MAX_PAGES_IN_MEMORY": 10,  # Увеличено количество страниц
    
    # Интервал очистки памяти (в секундах)
    "MEMORY_CLEANUP_INTERVAL": 60,  # Увеличено время между очистками
    
    # Таймаут для загрузки файла (в секундах)
    "FILE_UPLOAD_TIMEOUT": 900,  # 15 минут для загрузки
    
    # Таймаут для парсинга PDF (в секундах)
    "PDF_PARSING_TIMEOUT": 1200,  # 20 минут для парсинга
}
